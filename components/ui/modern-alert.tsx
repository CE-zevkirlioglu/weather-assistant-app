...
export function ModernAlert({
  ...
  const getGradientColors = (): readonly [string, string] => {
    switch (type) {
      case 'success':
        return isDark ? (['#064e3b', '#065f46'] as const) : (['#d1fae5', '#a7f3d0'] as const);
      case 'error':
        return isDark ? (['#7f1d1d', '#991b1b'] as const) : (['#fee2e2', '#fecaca'] as const);
      case 'warning':
        return isDark ? (['#78350f', '#92400e'] as const) : (['#fef3c7', '#fde68a'] as const);
      case 'info':
        return isDark ? (['#1e3a8a', '#1e40af'] as const) : (['#dbeafe', '#bfdbfe'] as const);
      default:
        return isDark ? (['#064e3b', '#065f46'] as const) : (['#d1fae5', '#a7f3d0'] as const);
    }
  };
